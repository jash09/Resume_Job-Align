...
df_resumes = pd.read_csv('resume_job_recs_gpt.csv')
...
for index, row in df_resumes.iterrows():
    ...
    while attempts < MAX_RETRIES:
        try:
            response = fn.generate_resume_gpt(resume_json, job_description)
...
df_resumes.to_csv('new_resumes_gpt.csv', index=False)